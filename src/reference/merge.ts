...
export function mergeReports(
  ...
  // Deduplicate by test ID if enabled
  if (deduplicateTests) {
    const seen = new Map<string, Test>()
    for (const test of allTests) {
      if (test.id) {
        // Keep the latest result (or first if no preference)
        seen.set(test.id, test)
      } else {
        // Tests without IDs cannot be deduplicated
  ...
  // Merge or calculate summary
  let summary: Summary
  if (mergeSummary) {
    summary = calculateSummary(allTests)

    // Merge timing from all reports
    ...
    // Sum up the summaries without recalculating
    summary = sumSummaries(reports.map(r => r.results.summary))
  }

  // Merge environments based on strategy
